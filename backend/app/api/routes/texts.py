...
from app.services.library import (
    list_texts,
    list_versions,
    list_versions_available_for_reference,
)
...
@router.get(
    "/{text_slug}/versions/available", response_model=list[VersionSummary]
)
def get_available_versions(
    ...
    try:
        return list_versions_available_for_reference(session, text_slug, reference)
    except ResourceNotFoundError as error:
        raise HTTPException(status_code=404, detail=str(error)) from error
